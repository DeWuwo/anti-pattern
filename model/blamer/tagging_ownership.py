...
from model.blamer.dep_blamer import load_commits
...
def entry():
    ...
    with open(args.ownership, "r") as file:
        reader = csv.DictReader(file, ["Entity", "category", "id", "file path", "commits"])
        reader.__next__()
        for row in reader:
...
def get_entity_owner(base_commit: str, only_accompany_commits: str, ownership: str, out_path: str):
    ...
    dump_ent_commit_infos(break_changes, f"{out_path}/mixed_entities.csv")
    dump_ent_commit_infos(accompany_changes, f"{out_path}/pure_accompany_entities.csv")
